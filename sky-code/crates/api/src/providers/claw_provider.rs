...
impl AuthSource {
    pub fn from_env() -> Result<Self, ApiError> {
        let api_key = read_env_non_empty("FILANTHROPIC_API_KEY")?;
        let auth_token = read_env_non_empty("FILANTHROPIC_AUTH_TOKEN")?;
        match (api_key, auth_token) {
            ...
            (None, None) => Err(ApiError::missing_credentials(
                "Claw",
                &["FILANTHROPIC_AUTH_TOKEN", "FILANTHROPIC_API_KEY"],
            )),
        }
    }
...
impl AuthSource {
    pub fn from_env_or_saved() -> Result<Self, ApiError> {
        if let Some(api_key) = read_env_non_empty("FILANTHROPIC_API_KEY")? {
            return match read_env_non_empty("FILANTHROPIC_AUTH_TOKEN")? {
        ...
        if let Some(bearer_token) = read_env_non_empty("FILANTHROPIC_AUTH_TOKEN")? {
            return Ok(Self::BearerToken(bearer_token));
        }

        // In offline mode, allow no credentials (local proxy doesn't need auth)
        if is_offline_mode() {
            return Ok(Self::None);
        }
...
pub fn has_auth_from_env_or_saved() -> Result<bool, ApiError> {
    Ok(read_env_non_empty("FILANTHROPIC_API_KEY")?.is_some()
        || read_env_non_empty("FILANTHROPIC_AUTH_TOKEN")?.is_some()
...
fn read_auth_token() -> Option<String> {
    read_env_non_empty("FILANTHROPIC_AUTH_TOKEN")
        .ok()
...
pub fn read_base_url() -> String {
    std::env::var("FILANTHROPIC_BASE_URL").unwrap_or_else(|_| DEFAULT_BASE_URL.to_string())
}

/// Check if we're in offline mode (custom base URL that's not the official Anthropic API)
#[must_use]
pub fn is_offline_mode() -> bool {
    std::env::var("FILANTHROPIC_BASE_URL")
        .ok()
        .is_some_and(|url| !url.starts_with("https://api.anthropic.com"))
}

/// Build HTTP client with appropriate timeout for offline/online mode
fn build_http_client() -> reqwest::Client {
    // Check for custom timeout from environment variable
    let timeout = if let Ok(timeout_str) = std::env::var("FILANTHROPIC_TIMEOUT_SECS") {
        timeout_str
            .parse::<u64>()
            .ok()
        ...
        // Offline mode (local proxy like Ollama) needs much longer timeout
        // because legacy versions (0.20.x) can be very slow (60-120s per request)
        OFFLINE_TIMEOUT
    } else {
        // Online mode (Anthropic API) uses default timeout
        DEFAULT_TIMEOUT
    };
...
mod tests {
    ...
    #[test]
    #[ignore] // Offline mode with Ollama doesn't require API keys
    fn read_api_key_requires_presence() {
        let _guard = env_lock();
        std::env::remove_var("FILANTHROPIC_AUTH_TOKEN");
        std::env::remove_var("FILANTHROPIC_API_KEY");
    ...
    fn read_api_key_requires_non_empty_value() {
        let _guard = env_lock();
        std::env::set_var("FILANTHROPIC_AUTH_TOKEN", "");
        std::env::remove_var("FILANTHROPIC_API_KEY");
    ...
    fn read_api_key_prefers_api_key_env() {
        let _guard = env_lock();
        std::env::set_var("FILANTHROPIC_AUTH_TOKEN", "auth-token");
        std::env::set_var("FILANTHROPIC_API_KEY", "legacy-key");
        assert_eq!(
