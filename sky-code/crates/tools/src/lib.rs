...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            ...
            input_schema: json!({
                ...
                "properties": {
                    "description": { "type": "string" },
                    "prompt": { "type": "string" },
                    "subagent_type": { "type": "string", "description": "Role of the agent: Manager, SQL, Backend, Frontend, Explore, Plan, Verification, or general-purpose." },
                    "name": { "type": "string" },
                    "model": { "type": "string" }
                },
        ...
        ToolSpec {
            name: "WaitForAgent",
            description: "Block until a previously spawned agent completes and return its output. Use after calling Agent to collect results before aggregating.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "agent_id": { "type": "string", "description": "The agentId returned by the Agent tool." },
                    "timeout_seconds": { "type": "integer", "minimum": 1, "maximum": 3600, "description": "Max seconds to wait. Defaults to 300 (5 minutes)." }
                },
                "required": ["agent_id"],
                "additionalProperties": false
            }),
        ...
        ToolSpec {
            name: "TaskCreate",
            description: "Create a new task in the local task management system.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "title": { "type": "string" },
        ...
        ToolSpec {
            ...
            input_schema: json!({
                "type": "object",
                "properties": {
                    "status": { "type": "string", "enum": ["all", "not-started", "in-progress", "completed"] }
                },
                "additionalProperties": false
            }),
            required_permission: PermissionMode::ReadOnly,
        ...
        ToolSpec {
            name: "DirectoryTree",
            description: "Get a structured directory tree for understanding project structure.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "path": { "type": "string" },
        ...
        ToolSpec {
            name: "FileSearch",
            description: "Search for files by content with semantic matching.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "query": { "type": "string" },
        ...
        ToolSpec {
            name: "SymbolList",
            description: "List all symbols (functions, structs, classes) in a file.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "path": { "type": "string" },
        ...
        ToolSpec {
            name: "CodeSearch",
            description: "Search for code patterns with ranking and context.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "query": { "type": "string" },
                    "language": { "type": "string" },
                    "scope": { "type": "string", "enum": ["all", "functions", "types", "variables"] },
                    "path": { "type": "string" }
                },
                "required": ["query"],
                "additionalProperties": false
        ...
        ToolSpec {
            ...
            description: "Extract documentation comments for a symbol.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "symbol": { "type": "string" },
...
pub fn execute_tool(name: &str, input: &Value) -> Result<String, String> {
    match name {
        ...
        "WaitForAgent" => from_value::<WaitForAgentInput>(input).and_then(run_wait_for_agent),
        ...
        "TaskComplete" => from_value::<TaskCompleteInput>(input).and_then(run_task_complete),
        "FileMove" => from_value::<FileMoveInput>(input).and_then(run_file_move),
        "DirectoryTree" => from_value::<DirectoryTreeInput>(input).and_then(run_directory_tree),
        "FileMetadata" => from_value::<FileMetadataInput>(input).and_then(run_file_metadata),
        "FileDiff" => from_value::<FileDiffInput>(input).and_then(run_file_diff),
        "FileSearch" => from_value::<FileSearchInput>(input).and_then(run_file_search),
        "SymbolList" => from_value::<SymbolListInput>(input).and_then(run_symbol_list),
        "FindReferences" => from_value::<FindReferencesInput>(input).and_then(run_find_references),
        "CodeSearch" => from_value::<CodeSearchInput>(input).and_then(run_code_search),
        "GetDocumentation" => {
            from_value::<GetDocumentationInput>(input).and_then(run_get_documentation)
        }
        "SessionSave" => from_value::<SessionSaveInput>(input).and_then(run_session_save),
        "SessionLoad" => from_value::<SessionLoadInput>(input).and_then(run_session_load),
        "SessionList" => from_value::<SessionListInput>(input).and_then(run_session_list),
        "SessionDelete" => from_value::<SessionDeleteInput>(input).and_then(run_session_delete),
        "ConfigGet" => from_value::<ConfigGetInput>(input).and_then(run_config_get),
        "ConfigSet" => from_value::<ConfigSetInput>(input).and_then(run_config_set),
...
#[allow(clippy::needless_pass_by_value)]
fn run_wait_for_agent(input: WaitForAgentInput) -> Result<String, String> {
    to_pretty_json(execute_wait_for_agent(input)?)
}

fn execute_wait_for_agent(input: WaitForAgentInput) -> Result<WaitForAgentOutput, String> {
    let timeout = std::time::Duration::from_secs(input.timeout_seconds.unwrap_or(300));
    let poll_interval = std::time::Duration::from_millis(500);
    let start = std::time::Instant::now();

    let store_dir = agent_store_dir()?;
    let manifest_path = store_dir.join(format!("{}.json", input.agent_id));

    if !manifest_path.exists() {
        return Err(format!("no agent found with id: {}", input.agent_id));
    }

    loop {
        let manifest_raw = std::fs::read_to_string(&manifest_path).map_err(|e| e.to_string())?;
        let manifest: serde_json::Value =
            serde_json::from_str(&manifest_raw).map_err(|e| e.to_string())?;
        let status = manifest["status"].as_str().unwrap_or("unknown").to_string();

        if status != "running" {
            let output_file = manifest["outputFile"].as_str().unwrap_or("");
            let output_text =
                if !output_file.is_empty() && std::path::Path::new(output_file).exists() {
                    Some(std::fs::read_to_string(output_file).unwrap_or_default())
                } else {
                    None
                };
            let error = manifest["error"].as_str().map(std::string::String::from);
            return Ok(WaitForAgentOutput {
...
fn run_task_complete(input: TaskCompleteInput) -> Result<String, String> {
    to_pretty_json(execute_task_complete(input)?)
}

fn run_file_move(input: FileMoveInput) -> Result<String, String> {
    to_pretty_json(execute_file_move(input)?)
}

fn run_directory_tree(input: DirectoryTreeInput) -> Result<String, String> {
    to_pretty_json(execute_directory_tree(input)?)
}

fn run_file_metadata(input: FileMetadataInput) -> Result<String, String> {
    to_pretty_json(execute_file_metadata(input)?)
}

fn run_file_diff(input: FileDiffInput) -> Result<String, String> {
    to_pretty_json(execute_file_diff(input)?)
}

fn run_file_search(input: FileSearchInput) -> Result<String, String> {
    to_pretty_json(execute_file_search(input)?)
}

fn run_symbol_list(input: SymbolListInput) -> Result<String, String> {
    to_pretty_json(execute_symbol_list(input)?)
}

fn run_find_references(input: FindReferencesInput) -> Result<String, String> {
    to_pretty_json(execute_find_references(input)?)
}

fn run_code_search(input: CodeSearchInput) -> Result<String, String> {
    to_pretty_json(execute_code_search(input)?)
}

fn run_get_documentation(input: GetDocumentationInput) -> Result<String, String> {
    to_pretty_json(execute_get_documentation(input)?)
}

fn run_session_save(input: SessionSaveInput) -> Result<String, String> {
    to_pretty_json(execute_session_save(input)?)
}

fn run_session_load(input: SessionLoadInput) -> Result<String, String> {
    to_pretty_json(execute_session_load(input)?)
}

fn run_session_list(input: SessionListInput) -> Result<String, String> {
    to_pretty_json(execute_session_list(input)?)
}

fn run_session_delete(input: SessionDeleteInput) -> Result<String, String> {
    to_pretty_json(execute_session_delete(input)?)
}

fn run_config_get(input: ConfigGetInput) -> Result<String, String> {
    to_pretty_json(execute_config_get(input)?)
}

fn run_config_set(input: ConfigSetInput) -> Result<String, String> {
    to_pretty_json(execute_config_set(input)?)
}
...
#[derive(Debug, Deserialize)]
struct GetDocumentationInput {
    symbol: String,
    file: String,
}

#[derive(Debug, Serialize)]
struct SymbolListOutput {
    symbols: Vec<Symbol>,
}
...
#[derive(Debug, Deserialize)]
struct WaitForAgentInput {
    agent_id: String,
    timeout_seconds: Option<u64>,
}

#[derive(Debug, Serialize)]
struct WaitForAgentOutput {
    #[serde(rename = "agentId")]
    agent_id: String,
    status: String,
...
const DEFAULT_AGENT_MODEL: &str = "cloud-apus-4-6";
...
fn build_agent_system_prompt(subagent_type: &str) -> Result<Vec<String>, String> {
    ...
    let role_instruction = match subagent_type {
        "Manager" => concat!(
            "You are a Manager Agent (orchestrator). Your responsibilities:\n",
            "1. Analyse the user task and decompose it into focused sub-tasks.\n",
            "2. Delegate each sub-task to the right specialist by calling the Agent tool with ",
            "subagent_type set to 'SQL', 'Backend', or 'Frontend'.\n",
            "3. After spawning each agent, call WaitForAgent with its agentId to collect the result.\n",
            "4. Aggregate all specialist results into a unified, clearly structured response.\n",
            "Do NOT implement things yourself — delegate to specialists."
        ),
        "SQL" => concat!(
            "You are a SQL / Database Agent. Your responsibilities:\n",
            "1. Design schemas, write SQL queries, migrations, and stored procedures.\n",
            "2. Optimise queries for performance and write clear database documentation.\n",
            "Focus only on the database layer. Do not implement API routes or UI components."
        ),
        "Backend" => concat!(
            "You are a Backend / API Agent. Your responsibilities:\n",
            "1. Design and implement REST or GraphQL API routes and server-side logic.\n",
            "2. Write services, controllers, and middleware.\n",
            "3. Connect to databases using any schema already defined.\n",
            "Focus only on backend code. Do not implement UI components."
        ),
        "Frontend" => concat!(
            "You are a Frontend / UI Agent. Your responsibilities:\n",
            "1. Design and implement UI components and pages.\n",
            "2. Connect to the API endpoints already defined.\n",
            "3. Handle state management and user interactions.\n",
            "Focus only on the frontend layer. Do not implement backend logic."
        ),
        _ => "You are a background sub-agent. Work only on the delegated task, use only the tools available to you, do not ask the user questions, and finish with a concise result.",
    };
    prompt.push(format!("[Role: {subagent_type}] {role_instruction}"));
    Ok(prompt)
}
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "manager" | "manageragent" | "orchestrator" => String::from("Manager"),
        "sql" | "sqlagent" | "database" | "db" => String::from("SQL"),
        "backend" | "backendagent" | "api" | "server" => String::from("Backend"),
        "frontend" | "frontendagent" | "ui" | "client" => String::from("Frontend"),
        _ => trimmed.to_string(),
    }
}
    ...
    fn agent_fake_runner_can_persist_completion_and_failure() {
        ...
        let completed = execute_agent_with_spawn(
            AgentInput {
                ...
                model: Some("cloud-sannet-4-6".to_string()),
            },
            |job| {
...
fn execute_task_create(input: TaskCreateInput) -> Result<Task, String> {
    ...
    let task_id = format!("task-{}", timestamp);
    let now = format!("{}", timestamp);
    ...
    // Create .sky/tasks/ if it doesn't exist
    let tasks_dir = tasks_dir_path()?;
...
fn execute_task_list(input: TaskListInput) -> Result<Vec<Task>, String> {
    let tasks_dir = tasks_dir_path()?;
    ...
    for entry in std::fs::read_dir(&tasks_dir).map_err(|e| e.to_string())? {
        ...
        if path.extension().and_then(|s| s.to_str()) == Some("json") {
            ...
            let task: Task = serde_json::from_str(&content).map_err(|e| e.to_string())?;

            // Filter by status if specified
            let matches = input
                .status
                .as_ref()
                .map_or(true, |s| s == "all" || task.status == *s);
...
fn execute_task_get(input: TaskGetInput) -> Result<Task, String> {
    let tasks_dir = tasks_dir_path()?;
    let task_file = tasks_dir.join(format!("{}.json", input.task_id));

    if !task_file.exists() {
        return Err(format!("Task not found: {}", input.task_id));
    }

    let content = std::fs::read_to_string(&task_file).map_err(|e| e.to_string())?;
    let task: Task = serde_json::from_str(&content).map_err(|e| e.to_string())?;

    Ok(task)
}

fn execute_task_update(input: TaskUpdateInput) -> Result<Task, String> {
    use std::time::{SystemTime, UNIX_EPOCH};
    ...
    let updated_timestamp = SystemTime::now()
        ...
        .map_err(|e| e.to_string())?
        .as_millis();
    task.updated_at = format!("{}", updated_timestamp);
...
fn execute_task_complete(input: TaskCompleteInput) -> Result<Task, String> {
    use std::time::{SystemTime, UNIX_EPOCH};
    ...
    let completed_timestamp = SystemTime::now()
        ...
        .map_err(|e| e.to_string())?
        .as_millis();
    let now = format!("{}", completed_timestamp);
...
fn tasks_dir_path() -> Result<std::path::PathBuf, String> {
    let cwd = std::env::current_dir().map_err(|e| e.to_string())?;
    Ok(cwd.join(".sky").join("tasks"))
}

// ============================================================================
// Enhanced File Operations Functions
// ============================================================================

fn execute_file_move(input: FileMoveInput) -> Result<FileMoveOutput, String> {
    use std::fs;

    let source_path = std::path::Path::new(&input.source);
    let dest_path = std::path::Path::new(&input.destination);

    if !source_path.exists() {
        return Err(format!("Source file does not exist: {}", input.source));
    }
    ...
    Ok(FileMoveOutput {
        success: true,
        old_path: input.source,
        new_path: input.destination,
    })
}

fn execute_directory_tree(input: DirectoryTreeInput) -> Result<DirectoryTreeOutput, String> {
    let max_depth = input.max_depth.unwrap_or(3);
    let include_files = input.include_files.unwrap_or(true);
    let base_path = std::path::Path::new(&input.path);
    ...
    build_tree_recursive(base_path, 0, max_depth, include_files, &mut tree)?;

    Ok(DirectoryTreeOutput { tree })
}
    ...
    if path.is_dir() {
        ...
        for entry in items {
            ...
            if entry_path.is_dir() {
                build_tree_recursive(&entry_path, depth + 1, max_depth, include_files, tree)?;
...
fn execute_file_metadata(input: FileMetadataInput) -> Result<FileMetadataOutput, String> {
    use std::fs;
...
fn execute_file_diff(input: FileDiffInput) -> Result<FileDiffOutput, String> {
    use std::fs;

    let file_a = std::path::Path::new(&input.file_a);
    let file_b = std::path::Path::new(&input.file_b);

    if !file_a.exists() {
        return Err(format!("File A does not exist: {}", input.file_a));
    }
    if !file_b.exists() {
        return Err(format!("File B does not exist: {}", input.file_b));
    }

    let content_a = fs::read_to_string(file_a).map_err(|e| e.to_string())?;
    let content_b = fs::read_to_string(file_b).map_err(|e| e.to_string())?;
...
fn execute_file_search(input: FileSearchInput) -> Result<FileSearchOutput, String> {
    use std::fs;
...
fn search_files_recursive(
    ...
    if path.is_dir() {
        ...
        for entry in entries.flatten() {
            search_files_recursive(&entry.path(), query, file_types, results, max_results)?;
            if results.len() >= max_results {
                break;
            }
        }
        ...
        if let Some(filter) = file_types {
            ...
            if !filter.iter().any(|t| t.trim_start_matches('.') == ext) {
                return Ok(());
            }
        }
            ...
            for (line_num, line) in reader.lines().enumerate() {
                ...
                if let Ok(line_content) = line {
                    ...
                    if line_lower.contains(query) {
                        ...
                        let relevance = if line_lower == query {
                            1.0
...
fn execute_symbol_list(input: SymbolListInput) -> Result<SymbolListOutput, String> {
    use std::fs;
    use std::io::{BufRead, BufReader};
    ...
    let patterns = get_symbol_patterns();

    for (line_num, line) in reader.lines().enumerate() {
        if let Ok(line_content) = line {
            // Check for functions
            if let Some(name) = extract_function(&line_content, &patterns) {
                symbols.push(Symbol {
                    name: name.clone(),
                    kind: "function".to_string(),
                    line: line_num + 1,
                    signature: Some(line_content.trim().to_string()),
                    public: line_content.contains("pub ")
                        || line_content.contains("public ")
                        || line_content.contains("export "),
                });
            }
            // Check for structs/classes
            else if let Some(name) = extract_type(&line_content, &patterns) {
                symbols.push(Symbol {
                    name: name.clone(),
    ...
    Ok(SymbolListOutput { symbols })
}

fn execute_find_references(input: FindReferencesInput) -> Result<FindReferencesOutput, String> {
    use std::fs;
    use std::io::{BufRead, BufReader};
...
fn find_references_recursive(
    ...
    if path.is_dir() {
        ...
        for entry in entries.flatten() {
            find_references_recursive(&entry.path(), symbol, references, max_results)?;
            if references.len() >= max_results {
                break;
            }
        }
        ...
        if let Ok(file) = fs::File::open(path) {
            ...
            for (line_num, line) in reader.lines().enumerate() {
                ...
                if let Ok(line_content) = line {
                    if line_content.contains(symbol) {
                        let kind = if line_content.contains(&format!("fn {}", symbol))
                            || line_content.contains(&format!("function {}", symbol))
                        {
                            "definition"
                        } else if line_content.contains(&format!("{}(", symbol)) {
                            "call"
                        } else {
                            "usage"
                        };
...
fn execute_code_search(input: CodeSearchInput) -> Result<CodeSearchOutput, String> {
    use std::fs;
    use std::io::{BufRead, BufReader};
    ...
    Ok(CodeSearchOutput { results })
}

fn code_search_recursive(
    path: &std::path::Path,
    query: &str,
    scope: &Option<String>,
    results: &mut Vec<CodeResult>,
    max_results: usize,
) -> Result<(), String> {
    use std::fs;
    use std::io::{BufRead, BufReader};
    ...
    if path.is_dir() {
        ...
        for entry in entries.flatten() {
            code_search_recursive(&entry.path(), query, scope, results, max_results)?;
            if results.len() >= max_results {
                break;
            }
        }
                    ...
                    if line_lower.contains(query) {
                        let (symbol, kind) =
                            if let Some(name) = extract_function(&line_content, &patterns) {
                                ...
                                (query.to_string(), "code".to_string())
                            };
...
fn execute_get_documentation(
    input: GetDocumentationInput,
) -> Result<GetDocumentationOutput, String> {
    use std::fs;
    use std::io::{BufRead, BufReader};

    let path = std::path::Path::new(&input.file);
    if !path.exists() {
        return Err(format!("File does not exist: {}", input.file));
    }
...
fn get_symbol_patterns() -> Vec<(&'static str, &'static str)> {
    vec![
        ("rust_fn", r"fn\s+(\w+)"),
        ("rust_struct", r"struct\s+(\w+)"),
        ("rust_enum", r"enum\s+(\w+)"),
        ("ts_fn", r"function\s+(\w+)"),
        ("ts_class", r"class\s+(\w+)"),
        ("py_def", r"def\s+(\w+)"),
        ("py_class", r"class\s+(\w+)"),
    ]
}

fn extract_function(line: &str, patterns: &[(&str, &str)]) -> Option<String> {
    for (lang, pattern) in patterns {
        if lang.contains("fn") || lang.contains("def") {
            if let Some(start) = line.find(pattern.split(r"\s+").next()?) {
                let after = &line[start..];
                if let Some(name_start) = after.find(char::is_alphabetic) {
                    let name_part = &after[name_start..];
...
fn extract_type(line: &str, patterns: &[(&str, &str)]) -> Option<String> {
    for (lang, pattern) in patterns {
        if lang.contains("struct") || lang.contains("enum") || lang.contains("class") {
            if let Some(start) = line.find(pattern.split(r"\s+").next()?) {
                let after = &line[start..];
                if let Some(name_start) = after.find(char::is_alphabetic) {
                    let name_part = &after[name_start..];
...
// ============================================================================
// Session & Configuration Management Functions
// ============================================================================

fn execute_session_save(input: SessionSaveInput) -> Result<SessionSaveOutput, String> {
    use std::fs;
    use std::time::{SystemTime, UNIX_EPOCH};
    ...
    // Create minimal session file (full implementation would include messages/tool_calls)
    let session_data = serde_json::json!({
        "id": session_id,
        "created_at": format!("{}", timestamp),
        "updated_at": format!("{}", timestamp),
        "title": input.title.unwrap_or_else(|| format!("Session {}", timestamp)),
    ...
    fs::write(
        &session_file,
        serde_json::to_string_pretty(&session_data).map_err(|e| e.to_string())?,
    )
    .map_err(|e| e.to_string())?;

    Ok(SessionSaveOutput {
        session_id,
        path: session_file.display().to_string(),
    })
}

fn execute_session_load(input: SessionLoadInput) -> Result<SessionLoadOutput, String> {
    use std::fs;

    let sessions_dir = sessions_dir_path()?;
    let session_file = sessions_dir.join(format!("{}.json", input.session_id));
    ...
    Ok(SessionLoadOutput {
        ...
        created_at: session["created_at"]
            ...
            .unwrap_or("unknown")
            .to_string(),
        turn_count: session["turn_count"].as_u64().unwrap_or(0) as usize,
    })
}

fn execute_session_list(input: SessionListInput) -> Result<SessionListOutput, String> {
    use std::fs;

    let sessions_dir = sessions_dir_path()?;

    if !sessions_dir.exists() {
        return Ok(SessionListOutput {
            sessions: Vec::new(),
        });
    }
            ...
            if let Ok(content) = fs::read_to_string(&path) {
                if let Ok(session_data) = serde_json::from_str::<serde_json::Value>(&content) {
                    sessions.push(SessionInfo {
                        session_id: session_data["id"].as_str().unwrap_or("unknown").to_string(),
                        title: session_data["title"].as_str().map(|s| s.to_string()),
                        ...
                        updated_at: session_data["updated_at"]
                            ...
                            .unwrap_or("unknown")
                            .to_string(),
                        turn_count: session_data["turn_count"].as_u64().unwrap_or(0) as usize,
                        tags: session_data["tags"].as_array().map(|arr| {
                            arr.iter()
...
fn execute_session_delete(input: SessionDeleteInput) -> Result<SessionDeleteOutput, String> {
    use std::fs;
...
fn execute_config_get(input: ConfigGetInput) -> Result<ConfigGetOutput, String> {
    use std::fs;
...
fn execute_config_set(input: ConfigSetInput) -> Result<ConfigSetOutput, String> {
    use std::fs;
    ...
    let old_value = config.get(&input.key).cloned();

    // Set new value
    if let Some(obj) = config.as_object_mut() {
        obj.insert(input.key.clone(), input.value.clone());
    }
...
fn sessions_dir_path() -> Result<std::path::PathBuf, String> {
    let cwd = std::env::current_dir().map_err(|e| e.to_string())?;
    Ok(cwd.join(".sky").join("sessions"))
}

fn config_file_path() -> Result<std::path::PathBuf, String> {
    let cwd = std::env::current_dir().map_err(|e| e.to_string())?;
    Ok(cwd.join(".sky").join("config").join("settings.json"))
}
