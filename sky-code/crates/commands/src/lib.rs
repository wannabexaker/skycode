...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        name: "ask",
        aliases: &["chat"],
        summary: "Switch to chat mode (no tools, pure conversation)",
        argument_hint: None,
        resume_supported: false,
    },
    SlashCommandSpec {
        name: "agent",
        aliases: &[],
        summary: "Switch to agent mode (all tools enabled)",
        argument_hint: None,
        resume_supported: false,
    },
    SlashCommandSpec {
        name: "plan",
        aliases: &[],
        summary: "Run deep planning mode for a task",
        argument_hint: Some("[task]"),
        resume_supported: false,
    },
    SlashCommandSpec {
        name: "filtersoff",
        aliases: &[],
        summary: "Disable content safety filters (use responsibly)",
        argument_hint: None,
        resume_supported: false,
    },
    SlashCommandSpec {
        name: "filterson",
        aliases: &[],
        summary: "Enable content safety filters (default)",
        argument_hint: None,
        resume_supported: false,
    },
];
...
impl SlashCommand {
    ...
    pub fn parse(input: &str) -> Option<Self> {
        ...
        Some(match command {
            ...
            "filtersoff" => Self::FiltersOff,
            "filterson" => Self::FiltersOn,
...
pub fn handle_agents_slash_command(args: Option<&str>, cwd: &Path) -> std::io::Result<String> {
    ...
    match normalized {
        ...
        Some("status") => Ok(render_agents_status(cwd)),
        Some(sub) if sub.starts_with("result") => {
            let id = sub.trim_start_matches("result").trim();
            if id.is_empty() {
                return Ok("Usage: /agents result <agent-id>".to_string());
            }
            Ok(render_agent_result(cwd, id))
        }
...
fn agent_run_store_dir(cwd: &Path) -> std::path::PathBuf {
    if let Ok(val) = std::env::var("CLAW_AGENT_STORE") {
        return std::path::PathBuf::from(val);
    }
    if let Some(ancestor) = cwd.ancestors().nth(2) {
        let candidate = ancestor.join(".claw-agents");
        if candidate.exists() {
            return candidate;
        }
    }
    cwd.join(".claw-agents")
}

fn render_agents_status(cwd: &Path) -> String {
    let store = agent_run_store_dir(cwd);
    let entries = match std::fs::read_dir(&store) {
        Ok(e) => e,
        Err(_) => {
            return "No background agents found (store directory does not exist).".to_string()
        }
    };

    let mut manifests: Vec<serde_json::Value> = entries
        .flatten()
    ...
    if manifests.is_empty() {
        return "No background agents found.".to_string();
    }
    ...
    let mut lines = vec![
        "Background Agents".to_string(),
        format!("  Store            {}", store.display()),
        String::new(),
    ];
    for m in &manifests {
        let id = m["agentId"].as_str().unwrap_or("?");
        let name = m["name"].as_str().unwrap_or("?");
        let status = m["status"].as_str().unwrap_or("?");
        let role = m["subagentType"].as_str().unwrap_or("general");
        let created = m["createdAt"].as_str().unwrap_or("");
        lines.push(format!("  {id}"));
        lines.push(format!("    name:    {name}  [{role}]"));
...
fn render_agent_result(cwd: &Path, agent_id: &str) -> String {
    let store = agent_run_store_dir(cwd);
    let manifest_path = store.join(format!("{agent_id}.json"));

    if !manifest_path.exists() {
        return format!("No agent found with id: {agent_id}");
    }
    ...
    let manifest: serde_json::Value = match serde_json::from_str(&raw) {
        Ok(v) => v,
        Err(e) => return format!("Failed to parse manifest: {e}"),
    };
    ...
    if status == "running" {
        return format!("Agent {agent_id} is still running. Use /agents status to check progress.");
    }

    if output_file.is_empty() || !std::path::Path::new(output_file).exists() {
        if let Some(err) = manifest["error"].as_str() {
            return format!("Agent {agent_id} failed: {err}");
        }
        return format!("Agent {agent_id} output file not found.");
    }

    match std::fs::read_to_string(output_file) {
        Ok(content) => content,
        Err(e) => format!("Failed to read agent output: {e}"),
    }
}
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        "Agents".to_string(),
        "  Usage            /agents [list|status|result <id>]".to_string(),
        "  Direct CLI       claw agents".to_string(),
        "  Sources          .codex/agents, .claw/agents, $CODEX_HOME/agents".to_string(),
        "  Subcommands      list     — show configured agent definitions".to_string(),
        "                   status   — show running/completed background agents".to_string(),
        "                   result <id>  — print output of a completed agent".to_string(),
    ];
...
mod tests {
    ...
    fn parses_supported_slash_commands() {
        ...
        assert_eq!(
            SlashCommand::parse("/model apus"),
            Some(SlashCommand::Model {
                model: Some("apus".to_string()),
            })
        );
        assert_eq!(
    ...
    fn ignores_unknown_or_runtime_bound_slash_commands() {
        ...
        assert!(
            handle_slash_command("/model sannet", &session, CompactionConfig::default()).is_none()
        );
        assert!(handle_slash_command(
    ...
    fn branch_and_worktree_commands_manage_git_state() {
        ...
        // Git on Windows outputs paths with forward slashes, so normalize for comparison
        let worktree_str = worktree_path.to_str().expect("utf8 path").replace('\\', "/");
        assert!(
            listed_worktrees.contains(&worktree_str) || 
            listed_worktrees.contains("branch-worktree-linked"),
