...
impl EditSession {
    ...
    fn render(
        ...
        self.render_with_completions(out, base_prompt, vim_enabled, &[])
    }
    ...
    fn render_with_completions(
        ...
        // Syntax highlight: /command gets cyan color
        let highlighted = highlight_slash_command(text);

        // Ghost suggestion: find best completion candidate
        let ghost = if self.mode != EditorMode::Command && !completions.is_empty() {
            slash_command_ghost(text, self.cursor, completions)
        } else {
            None
        };

        write!(out, "{prompt}{highlighted}")?;
        if let Some(ref ghost_text) = ghost {
            // Dim gray ghost completion hint
            write!(out, "\x1b[2;90m{ghost_text}\x1b[0m")?;
        }

        let ghost_len = ghost.as_deref().map(|g| g.chars().count()).unwrap_or(0);
        let (cursor_row, cursor_col, total_lines) =
            self.cursor_layout_with_ghost(prompt.as_ref(), ghost_len);
    ...
    fn cursor_layout(&self, prompt: &str) -> (usize, usize, usize) {
        self.cursor_layout_with_ghost(prompt, 0)
    }

    fn cursor_layout_with_ghost(&self, prompt: &str, ghost_len: usize) -> (usize, usize, usize) {
...
impl LineEditor {
    ...
    pub fn read_line(&mut self) -> io::Result<ReadOutcome> {
        ...
        session.render_with_completions(
            &mut stdout,
            &self.prompt,
            self.vim_enabled,
            &self.completions,
...
/// Interactive arrow-key picker. Shows `items` as a numbered list with a
/// highlighted cursor row. Returns the selected item string, or `None` if the
/// user pressed Escape / Ctrl-C.
///
/// Layout printed to stdout (then erased on selection):
///   ┌─ Select model ──────────────────┐
///   │  1. llama3.2:1b  <-- active     │
///   │  2. sky-uncensored               │
///   └─────────────────────────────────┘
...
pub fn pick_from_list(
    ...
    let mut stdout = io::stdout();
    let mut selected: usize = active_idx.unwrap_or(0).min(items.len() - 1);
    let total = items.len();

    // MoveUp count needed to get back to title line:
    // cursor starts on footer (line total+2), title is line 1 → need total+1 MoveUps
    let move_up_count = total + 1;

    let render_list = |stdout: &mut dyn Write, sel: usize| -> io::Result<()> {
        // Title line
        write!(stdout, "\x1b[2m  {title}\x1b[0m\r\n")?;
        for (i, item) in items.iter().enumerate() {
            ...
            if i == sel {
                // highlighted row: bold cyan background indicator
                write!(
                    stdout,
                    "  \x1b[36;1m▶ {}. {}{}\x1b[0m\r\n",
                    i + 1,
    ...
    render_list(&mut stdout, selected)?;
    ...
    loop {
        ...
        match key.code {
            ...
            KeyCode::Enter => {
                // Erase the list before returning
                write!(stdout, "\r")?;
        ...
        // Re-render: move cursor back up to top of list
        write!(stdout, "\r")?;
        for _ in 0..move_up_count {
            queue!(stdout, Clear(ClearType::CurrentLine), MoveUp(1))?;
        }
...
/// Syntax-highlight a slash command with ANSI cyan color.
/// Only the `/command` part is colored, arguments remain default.
fn highlight_slash_command(text: &str) -> String {
    if !text.starts_with('/') {
        return text.to_string();
    }
    // Find end of command word (first space or end of string)
    let cmd_end = text.find(' ').unwrap_or(text.len());
    let cmd = &text[..cmd_end];
    let rest = &text[cmd_end..];
    // Cyan + bold for the command keyword
    format!("\x1b[1;36m{cmd}\x1b[0m{rest}")
}

/// Return the ghost (inline suggestion) suffix to display after cursor.
/// Returns only the part the user hasn't typed yet.
fn slash_command_ghost<'a>(
    text: &str,
    cursor: usize,
    completions: &'a [String],
) -> Option<&'a str> {
    // Only activate ghost when cursor is at end and text starts with /
    if cursor != text.len() || !text.starts_with('/') {
        return None;
    }
    // No ghost if there's already a space (user is typing arguments)
    if text.contains(' ') {
        return None;
    }
    let prefix = text;
    completions
        .iter()
        .find(|c| c.starts_with(prefix) && c.as_str() != prefix)
        .map(|c| &c[prefix.len()..])
}
