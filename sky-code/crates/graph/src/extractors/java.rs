...
impl Extractor for JavaExtractor {
    fn extract(&self, source: &str, file_path: &str) -> Result<(Vec<Node>, Vec<Edge>)> {
        ...
        for child in root.children(&mut cur) {
            match child.kind() {
                "import_declaration" => extract_import(&child, src, file_path, &module_id, &mut nodes, &mut edges),
                "class_declaration" | "interface_declaration" | "enum_declaration" => {
                    extract_class_like(&child, src, file_path, &module_id, &mut nodes, &mut edges)
                }
                _ => {}
            }
        }
