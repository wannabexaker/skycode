...
mod tests {
    ...
    fn loads_and_merges_claw_code_config_files_by_precedence() {
        ...
        fs::write(
            ...
            r#"{"model":"sannet","env":{"A2":"1"},"hooks":{"PreToolUse":["base"]},"permissions":{"defaultMode":"plan"}}"#,
        )
        ...
        fs::write(
            ...
            r#"{"model":"apus","permissionMode":"acceptEdits"}"#,
        )
        ...
        assert_eq!(
            loaded.get("model"),
            Some(&JsonValue::String("apus".to_string()))
        );
        assert_eq!(loaded.model(), Some("apus"));
        assert_eq!(
