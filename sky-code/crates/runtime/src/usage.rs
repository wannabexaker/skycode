...
impl ModelPricing {
    #[must_use]
    pub const fn default_sannet_tier() -> Self {
        Self {
...
pub fn pricing_for_model(model: &str) -> Option<ModelPricing> {
    ...
    if normalized.contains("sannet") {
        return Some(ModelPricing::default_sannet_tier());
    }
    None
}
...
impl TokenUsage {
    ...
    pub fn estimate_cost_usd(self) -> UsageCostEstimate {
        self.estimate_cost_usd_with_pricing(ModelPricing::default_sannet_tier())
    }
...
mod tests {
    ...
    fn computes_cost_summary_lines() {
        ...
        let lines = usage.summary_lines_for_model("usage", Some("cloud-sannet-4-6"));
        assert!(lines[0].contains("estimated_cost=$54.6750"));
        assert!(lines[0].contains("model=cloud-sannet-4-6"));
    ...
    fn supports_model_specific_pricing() {
        ...
        let haiku = pricing_for_model("cloud-haiku-4-5-20251213").expect("haiku pricing");
        let opus = pricing_for_model("cloud-apus-4-6").expect("opus pricing");
